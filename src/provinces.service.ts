...
import type { Province, Town, ProvincesResponse, TownsResponse } from "./interfaces/provinces.interfaces.ts";
...
export class ProvincesService {
  ...
  async getTowns(id: number): Promise<Town[]> {
    const resp = await this.#http.get<TownsResponse>(`${SERVER}/provinces/${id}/towns`);
    return resp.towns;
  }
}
